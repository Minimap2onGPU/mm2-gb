...
__device__ int64_t getGuessLeaf(uint64_t key, int64_t modelIndex, int64_t *err) {
...
__device__ void rmiLookup(uint64_t *minimizers, int64_t *pos_array, int len, int i) {
  if (i < len) {
    BatchMetadata bm;
    bm.qid = i;
...
__global__ void mmIdxGet(uint64_t *minimizers, int64_t *pos, int *num_hits, uint64_t* values_enc, int len) {
...
int main(int argc, char *argv[]) {
  ...
  hipLaunchKernelGGL(mmIdxGet, dimGrid, dimBlock, 0, 0, deviceMinimizer, deviceOutputLisaPos, deviceOutputNumHits, deviceValuesEnc, inputLength);
  hipLaunchKernelGGL(mmIdxGet, dimGrid, dimBlock, 0, 0, deviceMinimizer, deviceOutputLisaPos, deviceOutputNumHits, deviceValuesEnc, inputLength);
  hipLaunchKernelGGL(mmIdxGet, dimGrid, dimBlock, 0, 0, deviceMinimizer, deviceOutputLisaPos, deviceOutputNumHits, deviceValuesEnc, inputLength);
  hipLaunchKernelGGL(mmIdxGet, dimGrid, dimBlock, 0, 0, deviceMinimizer, deviceOutputLisaPos, deviceOutputNumHits, deviceValuesEnc, inputLength);
  hipLaunchKernelGGL(mmIdxGet, dimGrid, dimBlock, 0, 0, deviceMinimizer, deviceOutputLisaPos, deviceOutputNumHits, deviceValuesEnc, inputLength);
