...
struct mm_plmap_t { // parameter pass
...
int mm_map_states(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, \
                        mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname, mm_plmap_t *plmap)
{
	// NOTE: here a state machine will work on n_segs SEEDING->CHAINING->ALIGNMENT
...
int mm_map_prechain(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, \
                        mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname, mm_plmap_t *plmap)
{
    // FIXME: Try state machine
    ...
    return 0; // FIXME: return value depends on GPU's reply
}

void *mm_map_postchain(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, \
                        mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname, mm_plmap_t *plmap) 
{
    int rep_len = plmap->rep_len, qlen_sum = plmap->qlen_sum, n_regs0 = plmap->n_regs0, n_mini_pos = plmap->n_mini_pos;
	uint32_t hash = plmap->hash;
...
{
	// NOTE: here a worker will work on n_segs SEEDING->CHAINING->ALIGNMENT
	int i, j, rep_len, qlen_sum, n_regs0, n_mini_pos;
	int max_chain_gap_qry, max_chain_gap_ref, is_splice = !!(opt->flag & MM_F_SPLICE), is_sr = !!(opt->flag & MM_F_SR);
	uint32_t hash;
	int64_t n_a;
...
{ 
	// NOTE: this is no longer called in C
	mm_reg1_t *regs;
