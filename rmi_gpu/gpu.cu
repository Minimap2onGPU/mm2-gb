...
bool loadRMI() {
  string filename = "./10k_100m.fa_map-ont_minimizers_key_value_sorted_keys.rmi_PARAMETERS";
...
bool loadSortedArray() {
  string filename = "./10k_100m.fa_map-ont_minimizers_key_value_sorted_keys.uint64";
  std::ifstream infile(filename, std::ios::in | std::ios::binary);
  if (!infile.good()) {
    printf("%s file not found\n", filename.c_str());
    exit(0);
...
void loadBin(){
  std::ifstream f_size("./10k_100m.fa_map-ont_minimizers_key_value_sorted_size");
	f_size >> keys_size;
	f_size >> p_size;
  ...
  string f1_name = "./10k_100m.fa_map-ont_minimizers_key_value_sorted_pos_bin";
  string f2_name = "./10k_100m.fa_map-ont_minimizers_key_value_sorted_val_bin";
...
void loadMinimizer(uint64_t* minimizer, int array_size) {
  std::ifstream file("./10k_100m.fa_map-ont_minimizers", std::ios::binary);
...
void loadResult(int64_t* lisa_pos, int* num_hits, int array_size) {
  std::ifstream file1("./10k_100m.fa_map-ont_lisa_pos", std::ios::binary);
  file1.read(reinterpret_cast<char*>(lisa_pos), array_size * sizeof(uint64_t));
  file1.close();
  
  std::ifstream file2("./10k_100m.fa_map-ont_minimizers_num_hits", std::ios::binary);
...
__device__ int64_t getGuessLeaf(uint64_t key, int64_t modelIndex, int64_t *err, int64_t n, double *l1Parameters) {
...
__device__ void rmiLookup(uint64_t *minimizers, int64_t *pos_array, uint64_t* sorted_array, int len, int i, int64_t n, double l0Parameter0, double l0Parameter1, int64_t l1Size, double *l1Parameters) {
  if (i < len) {
    BatchMetadata bm;
    bm.qid = i;
    ...
    // GUESS_RMI_LEAF
    int64_t err;
    int64_t guess = getGuessLeaf(bm.key, bm.modelIndex, &err, n, l1Parameters);
    bm.first = guess - err;
...
__global__ void mmIdxGet(uint64_t *minimizers, int64_t *pos, int *num_hits, uint64_t* values_enc, uint64_t* sorted_array, int len, int64_t n, double l0Parameter0, double l0Parameter1, int64_t l1Size, double *l1Parameters) {
  int i = threadIdx.x + blockDim.x * blockIdx.x;
  
  rmiLookup(minimizers, pos, sorted_array, len, i, n, l0Parameter0, l0Parameter1, l1Size, l1Parameters);
...
int main(int argc, char *argv[]) {
  ...
  hipMalloc((void **) &deviceValuesEnc, keys_size * sizeof(uint64_t));
  ...
  hipMemcpy(deviceValuesEnc, values_enc, keys_size* sizeof(uint64_t), hipMemcpyHostToDevice);
  ...
  dim3 dimGrid(ceil(inputLength * 1.0 / BLOCK_SIZE), 1, 1);
  ...
  std::ofstream outfile1("./test/lisa_pos_gpu_result.dat");
  std::ofstream outfile2("./test/num_hits_gpu_result.dat");
